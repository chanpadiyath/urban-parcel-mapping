...
import { fetchRoads, EMPTY_ROADS, type RoadCollection } from "./data/roads";
import { geocodeAddress, type GeocodeResult } from "./data/google";
...
export default function App() {
  ...
  const [roads, setRoads] = useState<RoadCollection>(EMPTY_ROADS);
  const [geocode, setGeocode] = useState<
    { status: "idle" } | { status: "loading" } | { status: "done"; result: GeocodeResult | null }
  >({ status: "idle" });
  ...
  // real OSM road geometry for the "Roads" layer — best-effort, doesn't block the main load
  useEffect(() => {
    let cancelled = false;
    fetchRoads()
      .then((fc) => { if (!cancelled) setRoads(fc); })
      .catch(() => { if (!cancelled) setRoads(EMPTY_ROADS); });
    return () => { cancelled = true; };
  }, []);
  ...
  const selectedCentroid = useMemo<[number, number] | null>(() => {
    const f = selected ? index.get(selected.parcel_id) : null;
    return f ? centroid(f) : null;
  }, [selected, index]);
  ...
  const handleGeocodeSearch = useCallback(async () => {
    ...
    setGeocode({ status: "done", result });
    if (result) {
      const d = 0.0015; // ~150m half-width, enough to pan/zoom to the point
      setFocusBounds([[result.lon - d, result.lat - d], [result.lon + d, result.lat + d]]);
    }
  }, [query]);
