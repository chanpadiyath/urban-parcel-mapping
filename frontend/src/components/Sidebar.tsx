...
import type { GeocodeResult } from "../data/google";
import { fmtPct } from "../metrics";

type GeocodeState = { status: "idle" } | { status: "loading" } | { status: "done"; result: GeocodeResult | null };
      ...
      <Card title="Search">
        ...
        {p.hasQuery && (
          <div className="res">
            ...
            {p.results.length === 0 && (
              <div className="res__geo">
                {p.geocode.status !== "loading" && (
                  <button type="button" className="link" onClick={p.onGeocodeSearch}>
                    Search address instead (Google) →
                  </button>
                )}
                {p.geocode.status === "loading" && <span className="sb__note">Searching…</span>}
                {p.geocode.status === "done" && p.geocode.result && (
                  <p className="sb__note">Found: {p.geocode.result.formatted_address} — map panned there.</p>
                )}
                {p.geocode.status === "done" && !p.geocode.result && (
                  <p className="sb__note">No address match, or Google search isn't configured (needs a server-side API key).</p>
                )}
              </div>
            )}
          </div>
